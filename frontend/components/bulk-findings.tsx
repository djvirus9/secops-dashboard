...
export function BulkFindings({ ids, projects, done, pending }: { ids: string[]; projects: string[]; done: (message: string) => void; pending: (busy: boolean) => void }) {
  ...
  return <form className="space-y-3 rounded-xl border p-4 dark:border-gray-700" onSubmit={submit}>
    ...
    <div className="flex flex-wrap items-end gap-3">
      <label className="grid gap-1 text-sm">Bulk status<select aria-label="Bulk status" className="input" value={status} disabled={busy} onChange={e => { setStatus(e.target.value); setConfirmed(false); }}><option value="">Keep status</option>{["open", "investigating", "verification_pending", "resolved", "closed"].map(value => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></label>
      ...
      {mode === "assign" && <AssigneeSelect label="Bulk assignee" projects={projects} value={assignee} onChange={setAssignee} disabled={busy} required />}
