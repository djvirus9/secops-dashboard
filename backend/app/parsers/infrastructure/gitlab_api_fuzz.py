...
class GitLabAPIFuzzParser(BaseParser):
    name = "gitlab_api_fuzz"
    display_name = "GitLab API Fuzzing"
    category = ScannerCategory.DAST
    file_types = ['json']
    description = "GitLab API fuzzing report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GitLab API Fuzzing Finding"))))
