...
class GitLabDASTParser(BaseParser):
    name = "gitlab_dast"
    display_name = "GitLab DAST"
    category = ScannerCategory.DAST
    file_types = ['json']
    description = "GitLab DAST scanning report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GitLab DAST Finding"))))
