...
class KubescapeParser(BaseParser):
    name = "kubescape"
    display_name = "Kubescape"
    category = ScannerCategory.INFRASTRUCTURE
    file_types = ['json']
    description = "Kubescape Kubernetes security scanner"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Kubescape Finding"))))
