...
class GitLabContainerParser(BaseParser):
    name = "gitlab_container"
    display_name = "GitLab Container Scan"
    category = ScannerCategory.CONTAINER
    file_types = ['json']
    description = "GitLab container scanning report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GitLab Container Scan Finding"))))
