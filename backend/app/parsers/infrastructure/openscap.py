...
class OpenSCAPParser(BaseParser):
    name = "openscap"
    display_name = "OpenSCAP"
    category = ScannerCategory.INFRASTRUCTURE
    file_types = ['xml']
    description = "OpenSCAP vulnerability scanner"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "OpenSCAP Finding"))))
