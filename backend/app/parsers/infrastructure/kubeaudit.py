...
class KubeauditParser(BaseParser):
    name = "kubeaudit"
    display_name = "Kubeaudit"
    category = ScannerCategory.INFRASTRUCTURE
    file_types = ['json']
    description = "Kubeaudit Kubernetes cluster audit"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Kubeaudit Finding"))))
