...
class ChefInspecParser(BaseParser):
    name = "chef_inspec"
    display_name = "Chef InSpec"
    category = ScannerCategory.INFRASTRUCTURE
    file_types = ['json']
    description = "Chef InSpec compliance report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Chef InSpec Finding"))))
