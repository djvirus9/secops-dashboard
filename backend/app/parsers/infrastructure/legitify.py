...
class LegitifyParser(BaseParser):
    name = "legitify"
    display_name = "Legitify"
    category = ScannerCategory.INFRASTRUCTURE
    file_types = ['json']
    description = "Legitify GitHub/GitLab security scanner"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Legitify Finding"))))
