...
class GitLabDependencyParser(BaseParser):
    name = "gitlab_dependency"
    display_name = "GitLab Dependency Scan"
    category = ScannerCategory.SCA
    file_types = ['json']
    description = "GitLab dependency scanning report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GitLab Dependency Scan Finding"))))
