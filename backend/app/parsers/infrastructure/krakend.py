...
class KrakenDParser(BaseParser):
    name = "krakend_audit"
    display_name = "KrakenD Audit"
    category = ScannerCategory.INFRASTRUCTURE
    file_types = ['json']
    description = "KrakenD API Gateway audit scan"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "KrakenD Audit Finding"))))
