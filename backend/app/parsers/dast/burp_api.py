...
class BurpAPIParser(BaseParser):
    name = "burp_api"
    display_name = "Burp REST API"
    category = ScannerCategory.DAST
    file_types = ['json']
    description = "Burp Suite REST API scan results"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Burp REST API Finding"))))
