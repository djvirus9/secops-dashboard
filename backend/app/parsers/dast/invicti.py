...
class InvictiParser(BaseParser):
    name = "invicti"
    display_name = "Invicti (Netsparker)"
    category = ScannerCategory.DAST
    file_types = ['json', 'xml']
    description = "Invicti web application security scanner"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Invicti (Netsparker) Finding"))))
