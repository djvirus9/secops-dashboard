...
class BurpDastardlyParser(BaseParser):
    name = "burp_dastardly"
    display_name = "Burp Dastardly"
    category = ScannerCategory.DAST
    file_types = ['xml', 'json']
    description = "Burp Suite Dastardly CI/CD scanner"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Burp Dastardly Finding"))))
