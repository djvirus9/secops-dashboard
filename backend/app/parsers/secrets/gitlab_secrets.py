...
class GitLabSecretsParser(BaseParser):
    name = "gitlab_secrets"
    display_name = "GitLab Secret Detection"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GitLab Secret Detection Finding"))))
