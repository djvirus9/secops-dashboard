...
class GgshieldParser(BaseParser):
    name = "ggshield"
    display_name = "Ggshield"
    category = ScannerCategory.SECRETS
    file_types = ['json']
    description = "GitGuardian ggshield secret detection"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Ggshield Finding"))))
