...
class GithubSecretsParser(BaseParser):
    name = "github_secrets"
    display_name = "GitHub Secrets Detection"
    category = ScannerCategory.SECRETS
    file_types = ['json']
    description = "GitHub secret detection alerts"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GitHub Secrets Detection Finding"))))
