...
class NSPParser(BaseParser):
    name = "nsp"
    display_name = "Node Security Platform"
    category = ScannerCategory.SCA
    file_types = ['json']
    description = "Node Security Platform vulnerability scan"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Node Security Platform Finding"))))
