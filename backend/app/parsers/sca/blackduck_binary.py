...
class BlackDuckBinaryParser(BaseParser):
    name = "blackduck_binary"
    display_name = "Black Duck Binary Analysis"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Black Duck Binary Analysis Finding"))))
