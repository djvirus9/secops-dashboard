...
class KiuwanSCAParser(BaseParser):
    name = "kiuwan_sca"
    display_name = "Kiuwan SCA"
    category = ScannerCategory.SCA
    file_types = ['json', 'csv']
    description = "Kiuwan software composition analysis"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Kiuwan SCA Finding"))))
