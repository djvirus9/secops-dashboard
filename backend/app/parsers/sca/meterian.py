...
class MeterianParser(BaseParser):
    name = "meterian"
    display_name = "Meterian"
    category = ScannerCategory.SCA
    file_types = ['json']
    description = "Meterian software composition analysis"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Meterian Finding"))))
