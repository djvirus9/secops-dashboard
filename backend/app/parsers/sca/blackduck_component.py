...
class BlackDuckComponentParser(BaseParser):
    name = "blackduck_component"
    display_name = "Black Duck Component Risk"
    category = ScannerCategory.SCA
    file_types = ['json', 'csv']
    description = "Black Duck component risk analysis"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Black Duck Component Risk Finding"))))
