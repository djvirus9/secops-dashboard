...
class NancyParser(BaseParser):
    name = "nancy"
    display_name = "Nancy"
    category = ScannerCategory.SCA
    file_types = ['json']
    description = "Nancy Go dependency vulnerability scanner"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Nancy Finding"))))
