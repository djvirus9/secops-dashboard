...
class JFrogBinaryParser(BaseParser):
    name = "jfrog_binary"
    display_name = "JFrog Xray Binary Scan"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "JFrog Xray Binary Scan Finding"))))
