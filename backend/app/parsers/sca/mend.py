...
class MendParser(BaseParser):
    name = "mend"
    display_name = "Mend (WhiteSource)"
    category = ScannerCategory.SCA
    file_types = ['json']
    description = "Mend/WhiteSource SCA vulnerability report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Mend (WhiteSource) Finding"))))
