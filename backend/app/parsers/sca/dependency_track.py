...
class DependencyTrackParser(BaseParser):
    name = "dependency_track"
    display_name = "Dependency-Track"
    category = ScannerCategory.SCA
    file_types = ['json']
    description = "OWASP Dependency-Track vulnerability report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Dependency-Track Finding"))))
