...
class KiuwanParser(BaseParser):
    name = "kiuwan"
    display_name = "Kiuwan SAST"
    category = ScannerCategory.SAST
    file_types = ['json', 'csv']
    description = "Kiuwan static application security testing"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Kiuwan SAST Finding"))))
