...
class GithubSASTParser(BaseParser):
    name = "github_sast"
    display_name = "GitHub SAST"
    category = ScannerCategory.SAST
    file_types = ['json']
    description = "GitHub SAST code scanning report"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GitHub SAST Finding"))))
