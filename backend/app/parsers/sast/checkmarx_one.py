...
class CheckmarxOneParser(BaseParser):
    name = "checkmarx_one"
    display_name = "Checkmarx One"
    category = ScannerCategory.SAST
    file_types = ['json']
    description = "Checkmarx One unified SAST/SCA/KICS"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Checkmarx One Finding"))))
