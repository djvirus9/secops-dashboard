...
class HCLASoCParser(BaseParser):
    name = "hcl_asoc"
    display_name = "HCL AppScan on Cloud"
    category = ScannerCategory.SAST
    file_types = ['json', 'xml']
    description = "HCL AppScan on Cloud SAST"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "HCL AppScan on Cloud Finding"))))
