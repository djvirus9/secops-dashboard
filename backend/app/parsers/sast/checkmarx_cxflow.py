...
class CheckmarxCxFlowParser(BaseParser):
    name = "checkmarx_cxflow"
    display_name = "Checkmarx CxFlow"
    category = ScannerCategory.SAST
    file_types = ['json']
    description = "Checkmarx CxFlow SAST integration"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Checkmarx CxFlow Finding"))))
