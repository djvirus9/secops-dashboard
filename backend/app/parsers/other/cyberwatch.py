...
class CyberwatchParser(BaseParser):
    name = "cyberwatch"
    display_name = "Cyberwatch"
    category = ScannerCategory.OTHER
    file_types = ['json', 'csv']
    description = "Cyberwatch vulnerability management"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Cyberwatch Finding"))))
