...
class HumbleParser(BaseParser):
    name = "humble"
    display_name = "Humble"
    category = ScannerCategory.OTHER
    file_types = ['json']
    description = "Humble HTTP security header analysis"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Humble Finding"))))
