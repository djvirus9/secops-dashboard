...
class GCPArtifactParser(BaseParser):
    name = "gcp_artifact"
    display_name = "GCP Artifact Scan"
    category = ScannerCategory.CLOUD
    file_types = ['json']
    description = "Google Cloud Artifact vulnerability scan"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "GCP Artifact Scan Finding"))))
