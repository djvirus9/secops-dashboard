...
class CloudflareParser(BaseParser):
    name = "cloudflare_insights"
    display_name = "Cloudflare Insights"
    category = ScannerCategory.CLOUD
    file_types = ['json']
    description = "Cloudflare security insights"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Cloudflare Insights Finding"))))
