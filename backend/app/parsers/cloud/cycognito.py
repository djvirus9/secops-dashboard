...
class CycognitoParser(BaseParser):
    name = "cycognito"
    display_name = "Cycognito"
    category = ScannerCategory.CLOUD
    file_types = ['json']
    description = "Cycognito attack surface management"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Cycognito Finding"))))
