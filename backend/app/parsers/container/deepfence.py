...
class DeepfenceParser(BaseParser):
    name = "deepfence"
    display_name = "Deepfence ThreatMapper"
    category = ScannerCategory.CONTAINER
    file_types = ['json']
    description = "Deepfence ThreatMapper runtime security"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Deepfence ThreatMapper Finding"))))
