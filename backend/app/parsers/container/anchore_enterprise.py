...
class AnchoreEnterpriseParser(BaseParser):
    name = "anchore_enterprise"
    display_name = "Anchore Enterprise"
    category = ScannerCategory.CONTAINER
    file_types = ['json']
    description = "Anchore Enterprise policy and vulnerability check"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "Anchore Enterprise Finding"))))
