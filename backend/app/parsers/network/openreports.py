...
class OpenReportsParser(BaseParser):
    name = "openreports"
    display_name = "OpenReports"
    category = ScannerCategory.NETWORK
    file_types = ['json']
    description = "OpenReports vulnerability data"
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                title = item.get("title", item.get("name", item.get("rule_id", item.get("id", "OpenReports Finding"))))
